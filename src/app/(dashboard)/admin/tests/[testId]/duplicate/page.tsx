...
export default function DuplicateTestPage() {
  ...
  useEffect(() => {
    async function loadTest() {
      ...
      if (data) {
        setTestTitle((data.title || data.test_title) + " (Copy)");
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Duplicate Test</h1>
          <p className="text-slate-500 text-sm">Create a copy of this test with new settings or questions.</p>
        </div>
        <button
          onClick={handleSave}
        ...
        >
          {status === "saving" ? "Saving..." : "Duplicate Test"}
        </button>
      </div>
